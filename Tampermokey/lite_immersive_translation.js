// ==UserScript==
// @name         Immersive Translator Lite
// @description  Immersive Translator Lite plugin, using the OpenAI Responses API.
// @author       Joey
// @version      2026-03-23
// @match        *://*/*
...
(function () {
  ...
  function initTouchShortcuts() {
    ...
    if (!CONFIG.enableTouchShortcuts) {
      console.info('[LocalBlockTranslator] touch shortcuts disabled by config.');
      return;
    }
  ...
  function buildInstructionText(payload, baseInstructions, options) {
    ...
    const commonRule = [
      ...
      `## 3. Output Contract`,
      `Your *only* valid output is the direct translation of the user's text from ${payload.sourceLang} into natural, fluent **${payload.targetLang}**.`,
  ...
  async function postResponsesRequest(endpoint, signal, requestBody) {
    ...
    try {
      const json = await postViaGM(endpoint, signal, requestBody);
      console.info('[LocalBlockTranslator] transport=gm');
      ...
      console.warn(`[LocalBlockTranslator] transport fallback to fetch: ${getErrorMessage(error)}`);
      const json = await postViaFetch(endpoint, signal, requestBody);
      console.info('[LocalBlockTranslator] transport=fetch-fallback');
      if (CONFIG.debugResponseLog) {
        console.info('[LocalBlockTranslator] response JSON:\n' + JSON.stringify(json, null, 2));
      }
      return json;
  ...
  function parseApiJsonOrThrow(status, responseText) {
    const bodyText = typeof responseText === 'string' ? responseText : String(responseText || '');
    if (status < 200 || status >= 300) {
      throw new Error(`API HTTP ${status}: ${bodyText.slice(0, 600)}`);
    }

    try {
      return JSON.parse(bodyText);
    } catch {
      throw new Error(`API invalid JSON response: ${bodyText.slice(0, 200)}`);
    }
  }

  async function postViaGM(endpoint, signal, requestBody) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const requestDetails = {
        ...
        onload: (response) => {
          try {
            const parsed = parseApiJsonOrThrow(response?.status || 0, response?.responseText || '');
            resolveOnce(parsed);
          } catch (error) {
            rejectOnce(error);
          }
        ...
        ontimeout: () => {
          rejectOnce(new Error(`API request timeout after ${CONFIG.requestTimeoutMs}ms`));
        },
        onabort: () => {
          rejectOnce(createAbortError());
        }
      ...
      try {
        ...
        if (maybeHandle && typeof maybeHandle.then === 'function') {
          maybeHandle
            ...
            .catch((error) => {
              rejectOnce(new Error(`GM request failed: ${getErrorMessage(error)}`));
            });
        }
      } catch (error) {
        rejectOnce(createGMUnsupportedError(`GM transport invocation failed: ${getErrorMessage(error)}`));
      }
    });
  }

  async function postViaFetch(endpoint, signal, requestBody) {
  ...
  try {
    ...
    logInfoIf(
      CONFIG.debugHotkey,
      `[LocalBlockTranslator] bootstrap version=${SCRIPT_VERSION} build=${buildId} time=${new Date().toISOString()}`
    );
    initHotkeys();
    initTouchShortcuts();
    initLauncherButton();
